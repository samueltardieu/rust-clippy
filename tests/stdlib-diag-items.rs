...
fn parse_content(content: &str) ->Vec<String> {
    ...
    let path_re = Regex::new(r"^pub static ([A-Z_]+): PathLookup = (?:macro|type|value)_path!\((([a-z]+)::.*)\);").unwrap();
    let mut errors = vec![];
        ...
        if let Some(caps) = path_re.captures(line) {
            if ["alloc", "core", "std"].contains(&&caps[3]) {
                if !caps[1].starts_with("DIAG_ITEM_") {
                    errors.push(format!("Path `{}` for `{}` should start with `DIAG_ITEM`", &caps[1], &caps[2]));
                    continue;
                }
            }
            if let Some(upper) = caps[1].strip_prefix("DIAG_ITEM_") {
                let Some(comment) = comment_re.captures(prev) else {
                    errors.push(format!("Definition for `{}` should be preceded by PR-related comment", &caps[1]));
                    continue;
                };
                let upper_sym = comment[1].to_uppercase();
                if upper != upper_sym {
                    errors.push(format!("Path for symbol `{}` should be named `DIAG_ITEM_{}`", &comment[1], upper_sym));
                    continue;
                }
            }
        }
