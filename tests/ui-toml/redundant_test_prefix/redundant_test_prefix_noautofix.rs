...
fn test_foo() {
    ...
    //~[default]| HELP: to override `-D warnings` add `#[allow(clippy::redundant_test_prefix)]
...
fn test_bar() {
    //~[outside_cfg_test]^ redundant_test_prefix
    //~[outside_cfg_test]| HELP: consider function renaming
...
#[test]
fn test_1() {
    //~[outside_cfg_test]^ ERROR: redundant `test_` prefix in test function name
    //~[outside_cfg_test]| HELP: removing `test_` prefix will produce invalid function name
