...
use rustc_span::{Span, StableSourceFileId, Symbol};
...
impl<'tcx> LateLintPass<'tcx> for DeferredConstShadow<'tcx> {
    ...
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        if let ExprKind::Path(QPath::Resolved(
            ...
            && let Some(def_span) = cx.tcx.hir().span_if_local(*def_id)
    ...
    fn check_item(&mut self, _: &LateContext<'tcx>, item: &'tcx Item<'tcx>) {
        if matches!(item.kind, ItemKind::Const(..)) && !item.span.from_expansion() {
            self.consts.last_mut().unwrap().insert(item.ident.name, item);
        }
    }
}
