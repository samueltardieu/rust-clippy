        ...
        && fn_def.def_id() == last_def.def_id
    {
        ...
        if !(is_mutable(cx, self_expr) || self_type.is_ref()) {
                ...
                && let PatKind::Binding(_, _, ident, _) = pat.kind
            {
                ...
                // If we can't make the binding mutable, we will consume it by enclosing
                // the receiver into a block which makes it into a temporary.
                sugg.push((self_expr.span.shrink_to_lo(), String::from("{ ")));
                sugg.push((self_expr.span.shrink_to_hi(), String::from(" }")));
            }
        }
        span_lint_and_then(
            cx,
